...
// ==========================================
// TARGET PYTHON SERVER (TCP Audio Stream)
// ==========================================
String pythonServerIP = ""; 
const int pythonServerPort = 8001; 

// ==========================================
// PIN DEFINITIONS (BOOT-SAFE & MATCHING PDF)
// ==========================================
#define STATUS_LED 2

// ST7735 TFT Screen
#define TFT_CS   16  // RX2
#define TFT_DC   17  // TX2
#define TFT_RST  -1  // Hardware EN Pin

// Ultrasonic Sensor
#define TRIG_PIN 13
#define ECHO_PIN 14  // Safe pin + 1k/2k Resistor Divider

// Direct Motor Control Pins (L298N)
#define IN1 19  // Right Wheels Forward
#define IN2 21  // Right Wheels Backward
#define IN3 22  // Left Wheels Forward
#define IN4 4   // Left Wheels Backward
...
void applyMotors(String dir) {
  ...
  // Safe stop before direction switch
...
void distanceTask(void * pvParameters) {
  while (true) {
    ...
    // 🚀 RED FACE THREAT DETECTION & EVASION
    if (currentMovementDir == "forward" && currentDistanceCM < 35.0 && currentDistanceCM > 2.0 && !isEvading) {
      isEvading = true; // Block external dashboard commands during maneuver
      
      Serial.printf("[AUTO THREAT] Obstacle detected at %.1f cm! Showing RED FACE...\n", currentDistanceCM);
      
      // 1. Trigger Red Angry Face Alert
      currentEmotion = ANGRY;
      renderFace();
      playTone(18000, 800); // Warning tone
      
      // 2. Kill forward momentum by braking slightly backward
      applyMotors("backward");
      vTaskDelay(150 / portTICK_PERIOD_MS);
      
      // 3. Hard evasive right turn
      applyMotors("right"); 
      vTaskDelay(800 / portTICK_PERIOD_MS);
      
      // 4. Stop motors and return face to Happy
      applyMotors("stop");
      currentEmotion = HAPPY;
      renderFace();
      
      isEvading = false; // Release lock
    }
...
void setup() {
  ...
  // SPI Setup (Free GPIO 19 from MISO conflict)
  SPI.begin(18, -1, 23, -1);
  ...
  xTaskCreatePinnedToCore(distanceTask, "DistTask", 2048, NULL, 1, NULL, 0);
}
